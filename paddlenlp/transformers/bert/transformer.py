...
from paddlenlp.utils.random import get_rng_state_tracker
...
class MultiHeadAttention(Layer):
    ...
    def __init__(self,
        ...
        self.embed_dim = embed_dim
        self.embed_dim_per_part = embed_dim // self.num_partitions
        self.num_heads_per_part = self.num_heads // self.num_partitions
        self.weight_attr = weight_attr
        self.bias_attr = bias_attr

        if num_partitions == 1:
            self.q_proj = Linear(
                embed_dim, self.embed_dim_per_part, weight_attr, bias_attr=bias_attr)
            self.k_proj = Linear(
                self.kdim, self.embed_dim_per_part, weight_attr, bias_attr=bias_attr)
            self.v_proj = Linear(
                self.vdim, self.embed_dim_per_part, weight_attr, bias_attr=bias_attr)
    ...
    def _prepare_qkv(self, query, key, value, cache=None):
        ...
        if self.num_partitions == 1:
            ...
            q = paddle.distributed.split(query,
                                         size=(self.embed_dim, self.embed_dim),
                                         operation='linear',
                                         axis=1,
                                         gather_out=False,
        ...
        if self.num_partitions == 1:
            ...
            q = tensor.reshape(x=q, shape=[0, 0, self.num_heads_per_part, self.head_dim])
    ...
    def compute_kv(self, key, value):
        ...
        if self.num_partitions == 1:
            k = self.k_proj(key)
            v = self.v_proj(value)
            # paddle.static.Print(k)
            # paddle.static.Print(v)
            k = tensor.reshape(x=k, shape=[0, 0, self.num_heads, self.head_dim])
            k = tensor.transpose(x=k, perm=[0, 2, 1, 3])
            v = tensor.reshape(x=v, shape=[0, 0, self.num_heads, self.head_dim])
            v = tensor.transpose(x=v, perm=[0, 2, 1, 3])
        else:
            k = paddle.distributed.split(key,
                                         size=(self.kdim, self.embed_dim),
                                         operation='linear',
                                         axis=1,
                                         gather_out=False,
                                         num_partitions=self.num_partitions,
                                         weight_attr=self.weight_attr,
                                         bias_attr = self.bias_attr)
            v = paddle.distributed.split(value,
                                         size=(self.vdim, self.embed_dim),
                                         operation='linear',
                                         axis=1,
                                         gather_out=False,
                                         num_partitions=self.num_partitions,
            ...
            # paddle.static.Print(k)
            # paddle.static.Print(v)
            k = tensor.reshape(x=k, shape=[0, 0, self.num_heads_per_part, self.head_dim])
            k = tensor.transpose(x=k, perm=[0, 2, 1, 3])
            v = tensor.reshape(x=v, shape=[0, 0, self.num_heads_per_part, self.head_dim])
            v = tensor.transpose(x=v, perm=[0, 2, 1, 3])
        # paddle.static.Print(k)
        # paddle.static.Print(v)
        return k, v
    ...
    def forward(self, query, key, value, attn_mask=None, cache=None):
        ...
        # paddle.static.Print(product)
        if attn_mask is not None:
            # TODO(guosheng): support bool mask
            product = product + attn_mask
        # paddle.static.Print(product)
        weights = F.softmax(product)
        # paddle.static.Print(weights)
        if self.dropout:
        ...
        if self.num_partitions == 1:
            ...
            out = paddle.distributed.split(out,
                                           size=(self.embed_dim, self.embed_dim),
                                           operation='linear',
                                           axis=0,
                                           gather_out=True,
...
class TransformerEncoderLayer(Layer):
    ...
    def __init__(self,
        ...
        weight_attrs = _convert_param_attr_to_list(weight_attr, 4)
        bias_attrs = _convert_param_attr_to_list(bias_attr, 4)
        self.weight_attr = weight_attrs
        self.bias_attr = bias_attrs
        ...
        self.norm1 = LayerNorm(d_model, epsilon=1e-05, weight_attr=False, bias_attr=False)
        self.norm2 = LayerNorm(d_model, epsilon=1e-05, weight_attr=False, bias_attr=False)
    ...
    def forward(self, src, src_mask=None):
        ...
        """
        residual = src
        # paddle.static.Print(residual, message="residual")
        if self.normalize_before:
            src = self.norm1(src)
        # TODO(guosheng): Add cache for encoder for the usage like UniLM
        # paddle.static.Print(src, message="src before attention")
        src = self.self_attn(src, src, src, src_mask)
        # paddle.static.Print(src, message="src after attention")
        with get_rng_state_tracker().rng_state('global_seed'):
            src = residual + self.dropout1(src)
        # paddle.static.Print(src, message="src after add residual")
        if not self.normalize_before:
            src = self.norm1(src)
        # paddle.static.Print(src, message="src after norm1 here")

        residual = src
        if self.normalize_before:
            src = self.norm2(src)
        # paddle.static.Print(src)
        # paddle.static.Print(src, message="src before linear1")
        if self.num_partitions == 1:
            src = self.linear1(src)
        else:
            src = paddle.distributed.split(src,
                                           size=(self.d_model, self.dim_feedforward),
                                           operation='linear',
                                           axis=1,
                                           gather_out=False,
                                           num_partitions=self.num_partitions,
                                           weight_attr=self.weight_attr[1],
                                           bias_attr=self.bias_attr[1])
        # paddle.static.Print(src, message="after linear1")
        if self.num_partitions == 1:
            src = self.linear2(self.dropout(self.activation(src)))
        else:
            src = self.activation(src)
            with get_rng_state_tracker().rng_state('local_seed'):
                src = self.dropout(src)
            src = paddle.distributed.split(src,
                                           size=(self.dim_feedforward, self.d_model),
                                           operation='linear',
                                           axis=0,
                                           num_partitions=self.num_partitions,
                                           weight_attr=self.weight_attr[1],
                                           bias_attr=self.bias_attr[1])
        # paddle.static.Print(src, message="after linear2")
        src = residual + self.dropout2(src)
        # paddle.static.Print(src)
        if not self.normalize_before:
            src = self.norm2(src)
        # paddle.static.Print(src, message="after norm2")
        return src
