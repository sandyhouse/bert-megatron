...
class BertPooler(Layer):
    ...
    def forward(self, hidden_states):
        ...
        # paddle.static.Print(first_token_tensor)
        pooled_output = self.dense(first_token_tensor)
        # paddle.static.Print(pooled_output)
...
class BertModel(BertPretrainedModel):
    ...
    def forward(self,
        ...
        if self.num_partitions > 1:
            ...
            paddle.distributed.broadcast(token_type_ids, src=0)
        embedding_output = self.embeddings(
            input_ids=input_ids,
            position_ids=position_ids,
            token_type_ids=token_type_ids)
        # paddle.static.Print(embedding_output, summarize=-1)
        encoder_outputs = self.encoder(embedding_output, attention_mask)
        # paddle.static.Print(encoder_outputs)
...
class BertLMPredictionHead(Layer):
    ...
    def forward(self, hidden_states, masked_positions=None):
        ...
        hidden_states = paddle.tensor.matmul(
            ...
            transpose_y=False) + self.decoder_bias
...
class BertPretrainingHeads(Layer):
    def __init__(self,
        ...
        self.seq_relationship = nn.Linear(hidden_size, 2, weight_attr=self.weight_attr, bias_attr=False)
...
class BertForPretraining(BertPretrainedModel):
    def __init__(self, bert, num_partitions):
