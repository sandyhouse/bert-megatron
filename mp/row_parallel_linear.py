...
def row_parallel_linear(
        ...
        bias_name=None,
        ring_id=0):
    assert in_size % mp_nranks == 0
    in_size_per_part = in_size // mp_nranks
    weight = paddle.create_parameter(shape=[in_size_per_part, out_size],
                                     dtype=dtype,
                                     name=param_name,
                                     attr=param_attr,
                                     is_bias=False)
    ...
    if use_bias:
        bias = paddle.create_parameter(shape=[out_size],
                                       dtype=dtype,
                                       name=bias_name,
                                       attr=param_attr,
                                       is_bias=True)
    ...
    if gather_out:
        paddle.distributed.all_reduce(out, group=ring_id)
    return out
